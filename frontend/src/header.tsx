...
export default function Header() {
  ...
  const getSearchedMovie = () => {
    fetch(
      `https://api.themoviedb.org/3/search/movie?query=${searchText.title}&api_key=649f645abf4721a3027659369cc67c24`
    )
  ...
  return (
    <div className="top">
      ...
      <div className="header">
        <div className="site--name" onClick={GoToHomePage}>
        ...
        {authToken ? (
          <div className="header--user" onClick={showUserProfile}>
            <img
              className="user--icon"
              src="https://drive.google.com/thumbnail?id=1dgRZJeX4kNcdsv2xQIFnvppTdNJBh3jp&sz=w1000"
              alt="no user"
            />
          </div>
        ) : (
      ...
      <Searched searchedMovie={searchedMovie} focused={focused} />
      <AuthForm />
      <Outlet />
    </div>
  );
